...
fn read_data() {
    ...
    assert_eq!(
        unwrapped_dataset.get(BitVec::<usize>::from_elem_general(9, true)),
        Some(221)
    );
    assert_eq!(
        unwrapped_dataset.get(BitVec::<usize>::from_elem_general(9, false)),
        Some(174)
    );
}
...
#[test]
fn read_nonascii() {
    let dataset = Dataset::read(Path::new("./tests/data/SCOTUS_n9_N11_nonascii.dat"));
    println!("{:?}", dataset);
    assert!(dataset.is_err())
}
