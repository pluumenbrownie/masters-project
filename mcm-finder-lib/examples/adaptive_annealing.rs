...
use mcm_finder_lib::solvers::{AdaptiveAnnealingSolver, AnnealingStarter, Solver};
...
fn main() -> Result<()> {
    ...
    let solver =
        AdaptiveAnnealingSolver::from_file(filepath)?.set_starter(AnnealingStarter::Trivial);
