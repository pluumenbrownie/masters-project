...
use mcm_finder_lib::solvers::{AdaptiveAnnealingSearcher, AdaptiveStarter, Solver};
...
fn main() -> Result<()> {
    ...
    let solver =
        AdaptiveAnnealingSearcher::from_file(filepath)?.set_starter(AdaptiveStarter::Trivial);
