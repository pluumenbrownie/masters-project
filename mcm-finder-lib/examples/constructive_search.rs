...
use mcm_finder_lib::solvers::{
    ...
    ConstructiveStrategy::{
        BackToFront, FrontToBack, GreedyOrder, RandomOrder, Tabu, VarianceFirst, VarianceLast,
    },
    Solver,
};
...
fn main() -> Result<()> {
    ...
    let solver = ConstructiveSolver::from_file(filepath)?.set_strategy(Tabu { steps: 100 });
